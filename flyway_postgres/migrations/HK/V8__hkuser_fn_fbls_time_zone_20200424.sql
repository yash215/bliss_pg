DROP FUNCTION IF EXISTS hkuser.fbls_time_zone();

CREATE OR REPLACE FUNCTION hkuser.fbls_time_zone(
	)
...
SELECT substring(timezone('Asia/Hong_Kong'::text, now())::text, 1, 16) INTO print_date_time;
