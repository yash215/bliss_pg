DROP FUNCTION IF EXISTS sguser.fbls_time_zone();

CREATE OR REPLACE FUNCTION sguser.fbls_time_zone(
	)
...
SELECT substring(timezone('Asia/Singapore'::text, now())::text, 1, 16) INTO print_date_time;
